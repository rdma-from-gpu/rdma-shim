...
// Here we have some helpers to access or manipulate rdma-core structs
// allowing to import this lib only instead of the whole mlx5 source files
...
void *sq_buf_buf_from_mlx5_qp(struct mlx5_qp *qp) { return qp->sq_buf.buf; }

void *sq_start_from_mlx5_qp(struct mlx5_qp *qp) { return qp->sq_start; }
...
uint64_t prepare_rdma(struct ibv_qp *qp, struct rdma_shim_data *data,
    ...
    if (lowest_p) *lowest_p = (void *)lowest;

    if (highest_p) *highest_p = (void *)highest;

    printf("Lowest address is %p, highest %p, total %liB\n", (void *)lowest,
           (void *)highest, highest - lowest);
...
// This implements a rdma_write on the CPU "crafted" version
void rdma_write(struct rdma_shim_data *data, uint64_t laddr, uint64_t raddr,
                uint32_t lkey, uint32_t rkey, uint32_t bytes) {
    ...
    ctrl_seg_ptr = (struct mlx5_wqe_ctrl_seg *)((uintptr_t)data->sq_start +
    ...
    raddr_seg_ptr =
        (struct mlx5_wqe_raddr_seg *)((uintptr_t)ctrl_seg_ptr + size);
...
struct mlx5_wqe_ctrl_seg *get_ctrl_seg_ptr(struct rdma_shim_data *data,
                                           int idx) {
...
void rdma_write_with_imm(struct rdma_shim_data *data, int imm) {
    ...
    ctrl_seg.opmod_idx_opcode =
        htobe32((idx << 8) | MLX5_OPCODE_RDMA_WRITE_IMM);
    ctrl_seg.imm = imm;
    ...
    // data_seg_ptr = (struct mlx5_wqe_data_seg*) ((uintptr_t)ctrl_seg_ptr +
    // size);
    // data_seg.addr = htobe64((uint64_t)laddr);
    // data_seg.byte_count = htobe32(bytes);
    // data_seg.lkey = htobe32(lkey);
...
int mlx5_bf_reg_size(struct rdma_shim_data *data) {
    return data->mctx->bf_reg_size;
}

// Helpers to advance the CQ registers
...
void *rdma_shim_malloc(int size) {
    if (size < (custom_malloc_buffer_size - allocated_malloc_buffer_size)) {
        void *ret = (void *)((uint64_t)custom_malloc_buffer +
                             allocated_malloc_buffer_size);
